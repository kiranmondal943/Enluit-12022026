...
# --- 1. APP CONFIGURATION ---
st.set_page_config(
    page_title="Titan v32.1 | Visual Fixes", 
    layout="wide", 
    page_icon="⚡",
    initial_sidebar_state="expanded"
)

# --- 2. ADVANCED UI SYSTEM (STREAMLIT INTERFACE) ---
st.markdown("""
    <style>
    ...
    .stButton>button {
        ...
        color: white; font-weight: 800; border: none; text-transform: uppercase; letter-spacing: 1px;
...
with st.sidebar:
    ...
    st.caption("v32.1 | Text Visibility Fixed")
    st.divider()
    ...
    with st.expander("🧩 Section Manager", expanded=False):
        ...
        show_inventory = st.checkbox("Portfolio/Inventory", value=True)
...
tabs = st.tabs(["1. Identity", "2. Content Blocks", "3. Pricing", "4. Inventory", "5. Blog", "6. Legal & Footer"])
...
with tabs[0]:
    ...
    with c2:
        ...
        seo_d = st.text_area("Meta Description", "Stop paying monthly fees. Titan Engine builds 0.1s websites.", height=100)
...
with tabs[1]:
    ...
    feat_data = st.text_area("Features List", "bolt | Performance | 0.1s Loading Speed\nwallet | Economics | $0 Monthly Fees\ntable | Control | Google Sheets CMS\nshield | Security | Zero-DB Architecture", height=100)
    
    st.subheader("About Content")
    about_h = st.text_input("About Title", "Control Your Empire")
    about_img = st.text_input("About Side Image", "https://images.unsplash.com/photo-1543286386-713df548e9cc?q=80&w=1600")
    about_short = st.text_area("Home Summary", "No WordPress dashboard. Just open your private Google Sheet.", height=100)
    about_long = st.text_area("Full About Page", "Most business owners are trapped in a rental cycle. We believe in ownership.", height=150)
...
with tabs[3]:
    ...
    sheet_url = st.text_input("Inventory CSV Link", placeholder="https://docs.google.com/spreadsheets/d/e/.../pub?output=csv")
...
with tabs[4]:
    ...
    blog_hero_sub = st.text_input("Blog Page Subtext", "Thoughts on technology and freedom.")

with tabs[5]:
    st.subheader("Legal")
    testi_data = st.text_area("Testimonials", "Rajesh Gupta | Titan stopped the bleeding.\nSarah Jenkins | Updating my menu is easy.", height=100)
    faq_data = st.text_area("FAQ Data", "Do I really pay $0? ? Yes, static hosting is free.\nIs it secure? ? Yes, no database to hack.", height=100)
...
def get_theme_css():
    ...
    * {{ box-sizing: border-box; }}
    ...
    .mobile-menu {{ display: none; font-size: 1.5rem; cursor: pointer; color: var(--txt); }}
    ...
    .detail-view {{ display: grid; grid-template-columns: 1fr 1fr; gap: 3rem; align-items: start; }}
    ...
    .pricing-wrapper {{ overflow-x: auto; -webkit-overflow-scrolling: touch; padding-bottom: 1rem; }}
    .pricing-table {{ width: 100%; border-collapse: collapse; min-width: 600px; }}
    .pricing-table th, .pricing-table td {{ padding: 1.2rem; text-align: left; border-bottom: 1px solid rgba(100,100,100,0.1); }}
    .pricing-table th {{ background: var(--p); color: white; }}
    .pricing-table td {{ background: var(--card); color: var(--txt); }}
    ...
    .footer a {{ color: rgba(255,255,255,0.7) !important; text-decoration: none; display: block; margin-bottom: 0.5rem; }}
    ...
    .share-row {{ display: flex; gap: 0.8rem; margin-top: 1.5rem; align-items: center; flex-wrap: wrap; }}
    ...
    .bg-fb {{ background: #1877F2; }} .bg-x {{ background: #000000; }} .bg-li {{ background: #0A66C2; }} .bg-link {{ background: #64748b; }}
    ...
    @media (max-width: 768px) {{
        ...
        .about-grid, .contact-grid, .detail-view, .grid-3 {{ grid-template-columns: 1fr; gap: 2rem; }}
        
        .about-grid img {{ order: -1; }} /* Image on top for about */
...
def gen_nav():
    logo = f'<img src="{logo_url}" height="32" alt="{biz_name}">' if logo_url else f'<span style="font-weight:900;font-size:1.3rem;color:var(--p)">{biz_name}</span>'
    ...
    <nav><div class="container nav-flex">
        ...
        <div class="nav-links">
            ...
            {'<a href="index.html#inventory" onclick="toggleMenu()">Portfolio</a>' if show_inventory else ''}
            {blog_link}
            <a href="about.html" onclick="toggleMenu()">About</a>
            <a href="contact.html" onclick="toggleMenu()">Contact</a>
            <a href="tel:{biz_phone}" class="btn-accent" style="padding:0.6rem 1.5rem; border-radius:50px; color:white!important; width:auto; text-align:center; display:inline-block;">Call Now</a>
        </div>
    </div></nav>
...
def get_simple_icon(name):
    ...
    path = icon_map.get(name, "M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z")
...
def gen_features():
    ...
    for line in lines:
        parts = line.split('|')
        if len(parts) >= 3:
            cards += f"""<div class="card reveal"><div style="color:var(--s); margin-bottom:1rem;">{get_simple_icon(parts[0])}</div><h3 style="color:var(--p); font-size:1.2rem; text-transform:uppercase; letter-spacing:1px;">{parts[1].strip()}</h3><div style="opacity:0.9; color:var(--txt); font-size:0.95rem;">{format_text(parts[2].strip())}</div></div>"""
...
def gen_csv_parser():
    ...
    function parseMarkdown(text) {
        ...
        if (html.includes('* ')) {
            const lines = html.split('<br>'); let inList = false; let finalHtml = '';
            lines.forEach(line => {
                if (line.trim().startsWith('* ')) { if (!inList) { finalHtml += '<ul>'; inList = true; } finalHtml += '<li>' + line.trim().substring(2) + '</li>'; }
                else { if (inList) { finalHtml += '</ul>'; inList = false; } finalHtml += line + '<br>'; }
            });
            if (inList) finalHtml += '</ul>'; return finalHtml;
        }
        return html;
    }
    </script>
        ...
        try {{
            ...
            for(let i=1; i<lines.length; i++) {{
                ...
                if(clean.length > 1) {{
                    ...
                    box.innerHTML += `<div class="card reveal"><img src="${{img}}" class="prod-img" loading="lazy"><div><h3 style="color:var(--p);font-size:1.2rem;">${{clean[0]}}</h3><p style="color:var(--s);font-weight:bold;">${{clean[1]}}</p><p style="font-size:0.9rem;opacity:0.8;">${{clean[2] ? clean[2].substring(0,60)+'...' : ''}}</p><a href="product.html?item=${{prodName}}" class="btn btn-primary" style="padding:0.5rem 1rem; width:100%;">View Details</a></div></div>`;
                }}
            }}
...
def gen_inventory():
    ...
    return f"""<section id="inventory" style="background:rgba(0,0,0,0.02)"><div class="container"><div class="section-head reveal"><h2>Portfolio</h2></div><div id="inv-grid" class="grid-3"><div style="grid-column:1/-1; text-align:center;">Loading...</div></div></div></section>{gen_inventory_js(is_demo=False)}"""

def gen_about_section():
    return f"""<section id="about"><div class="container"><div class="about-grid"><div class="reveal"><h2>{about_h}</h2><div style="opacity:0.9;">{format_text(about_short)}</div><a href="about.html" class="btn btn-primary" style="margin-top:1.5rem;">Read More</a></div><img src="{about_img}" class="reveal" style="width:100%; border-radius:var(--radius);"></div></div></section>"""
...
def gen_footer():
    return f"""<footer><div class="container"><div class="footer-grid"><div><h3>{biz_name}</h3><p style="color:rgba(255,255,255,0.7);">{biz_addr}</p><p style="color:rgba(255,255,255,0.7);">{biz_email}</p></div><div><h4>Explore</h4><a href="index.html">Home</a><a href="blog.html">Blog</a></div><div><h4>Legal</h4><a href="privacy.html">Privacy</a><a href="terms.html">Terms</a></div></div><div style="text-align:center; margin-top:3rem; opacity:0.4; font-size:0.8rem;">&copy; {biz_name}.</div></div></footer>"""

def gen_wa_widget():
    if not wa_num: return ""
    return f"""<a href="https://wa.me/{wa_num}" target="_blank" style="position:fixed; bottom:20px; right:20px; background:#25d366; color:white; width:55px; height:55px; border-radius:50%; display:flex; align-items:center; justify-content:center; box-shadow:0 10px 20px rgba(0,0,0,0.2); z-index:9999;"><svg style="width:30px;height:30px;fill:white;" viewBox="0 0 24 24"><path d="M12.04 2c-5.46 0-9.91 4.45-9.91 9.91c0 1.75.46 3.45 1.32 4.95L2.05 22l5.25-1.38c1.45.79 3.08 1.21 4.74 1.21c5.46 0 9.91-4.45 9.91-9.91c0-2.65-1.03-5.14-2.9-7.01A9.816 9.816 0 0 0 12.04 2m.01 1.67c2.2 0 4.26.86 5.82 2.42a8.225 8.225 0 0 1 2.41 5.83c0 4.54-3.7 8.23-8.24 8.23c-1.48 0-2.93-.39-4.19-1.15l-.3-.17l-3.12.82l.83-3.04l-.2-.32a8.188 8.188 0 0 1-1.26-4.38c.01-4.54 3.7-8.24 8.25-8.24m-3.53 3.16c-.13 0-.35.05-.54.26c-.19.2-.72.7-.72 1.72s.73 2.01.83 2.14c.1.13 1.44 2.19 3.48 3.07c.49.21.87.33 1.16.43c.49.16.94.13 1.29.08c.4-.06 1.21-.5 1.38-.98c.17-.48.17-.89.12-.98c-.05-.09-.18-.13-.37-.23c-.19-.1-.1.13-.1.13s-1.13-.56-1.32-.66c-.19-.1-.32-.15-.45.05c-.13.2-.51.65-.62.78c-.11.13-.23.15-.42.05c-.19-.1-.8-.3-1.53-.94c-.57-.5-1.02-1.12-1.21-1.45c-.11-.19-.01-.29.09-.38c.09-.08.19-.23.29-.34c.1-.11.13-.19.19-.32c.06-.13.03-.24-.01-.34c-.05-.1-.45-1.08-.62-1.48c-.16-.4-.36-.34-.51-.35c-.11-.01-.25-.01-.4-.01Z"/></svg></a>"""
...
def build_page(title, content, extra_js=""):
    return f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>{title} | {biz_name}</title>{gen_schema()}<link href="https://fonts.googleapis.com/css2?family={h_font.replace(' ', '+')}:wght@400;700;900&family={b_font.replace(' ', '+')}:wght@300;400;600&display=swap" rel="stylesheet"><style>{get_theme_css()}</style>{extra_js}</head><body>{gen_nav()}{content}{gen_footer()}{gen_wa_widget()}{gen_scripts()}</body></html>"""

# --- PAGES ---
def gen_product_page_content(is_demo=False):
    demo_flag = "const isDemo = true;" if is_demo else "const isDemo = false;"
    return f"""
    <section style="padding-top:120px;"><div class="container"><div id="product-detail" class="detail-view"><div style="background:#eee; height:400px; border-radius:12px;"></div><div>Loading...</div></div></div></section>
    {gen_csv_parser()}
    <script>{demo_flag}
    function shareWA(url) {{ window.open('https://wa.me/?text=' + encodeURIComponent(url), '_blank'); }}
                ...
                if(clean[0] === targetName) {{
                    ...
                    document.getElementById('product-detail').innerHTML = `
                        ...
                        <div>
                            ...
                            <p style="font-size:1.5rem; color:var(--s); font-weight:bold;">${{clean[1]}}</p>
                            <p style="opacity:0.9;">${{clean[2]}}</p>
                            <button onclick="shareWA(window.location.href)" class="btn btn-primary" style="width:100%; margin-top:2rem;">Buy on WhatsApp</button>
                            <div class="share-row">
                                <span style="font-weight:bold;font-size:0.9rem;">Share:</span>
...
def gen_blog_index_html():
    ...
    <section><div class="container"><div id="blog-grid" class="grid-3" style="min-height:200px;">Loading...</div></div></section>
    {gen_csv_parser()}
    <script>
    async function loadBlog() {{
        try {{
            ...
            for(let i=1; i<lines.length; i++) {{
                ...
                if(r.length > 4) {{
                    box.innerHTML += `<div class="card reveal"><img src="${{r[5] || '{hero_img_1}'}}" class="prod-img" alt="${{r[1]}}"><div><span class="blog-badge">${{r[3]}}</span><span style="float:right;font-size:0.8rem;opacity:0.6;">${{r[2]}}</span><h3 style="margin-top:0.5rem;font-size:1.2rem;"><a href="post.html?id=${{r[0]}}" style="text-decoration:none;color:inherit;">${{r[1]}}</a></h3><p style="font-size:0.9rem;opacity:0.8;">${{r[4]}}</p><a href="post.html?id=${{r[0]}}" style="color:var(--s);font-weight:bold;text-decoration:none;">Read More &rarr;</a></div></div>`;
                }}
            }}
        }} catch(e) {{}}
    }}
...
def gen_blog_post_html():
    ...
    <div id="post-container" style="padding-top:100px; min-height:60vh;">
        <div class="container" style="text-align:center; padding:5rem 0;"><h1>Loading...</h1></div>
    </div>
    {gen_csv_parser()}
    <script>
        ...
        try {{
            ...
            for(let i=1; i<lines.length; i++) {{
                ...
                if(r[0] === slug) {{
                    ...
                    container.innerHTML = `
                        <div style="background:var(--p); padding:6rem 0 4rem 0; color:white; text-align:center; width:100%;">
                            <div class="container">
                                <span class="blog-badge" style="background:rgba(255,255,255,0.2);">${{r[3]}}</span>
                                <h1 style="color:white; margin-bottom:1rem;">${{r[1]}}</h1>
                                <p style="opacity:0.8;">${{r[2]}}</p>
                            </div>
                        </div>
                        <div class="container" style="max-width:800px; padding:4rem 20px;">
                            <img src="${{r[5]}}" style="width:100%; border-radius:12px; margin-bottom:3rem; box-shadow:0 10px 30px rgba(0,0,0,0.1);">
                            <div class="article-content" style="line-height:1.8; color:var(--txt);">${{contentHtml}}</div>
...
preview_mode = st.radio("Preview Page:", ["Home", "About", "Contact", "Blog Index", "Blog Post", "Product Detail"], horizontal=True)

about_content = f"""<section class="hero" style="min-height: 40vh; background-image: linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), url('{hero_img_1}'); background-size: cover; background-position: center;"><div class="container"><h1>About Us</h1></div></section><section><div class="container"><div class="about-grid"><div class="legal-text">{format_text(about_long)}</div><img src="{about_img}" style="width:100%; border-radius:12px;"></div></div></section>"""

contact_content = f"""<section class="hero" style="min-height: 40vh; background-image: linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), url('{hero_img_1}'); background-size: cover; background-position: center;"><div class="container"><h1>Contact</h1></div></section>
<section><div class="container"><div class="contact-grid"><div><div style="background:var(--card); padding:2rem; border-radius:12px; border:1px solid #eee;"><h3>Get In Touch</h3><p>{biz_addr}</p><p><a href="tel:{biz_phone}" style="color:var(--s);">{biz_phone}</a></p><p>{biz_email}</p><br><a href="https://wa.me/{wa_num}" target="_blank" class="btn btn-accent" style="width:100%;">WhatsApp Us</a></div></div><div class="card"><h3>Send Message</h3><form action="https://formsubmit.co/{biz_email}" method="POST"><label>Name</label><input type="text" name="name" required><label>Email</label><input type="email" name="email" required><label>Message</label><textarea name="msg" rows="4" required></textarea><button class="btn btn-primary" type="submit">Send</button></form></div></div><br><div style="border-radius:12px;overflow:hidden;">{map_iframe}</div></div></section>"""
...
with c1:
    ...
    elif preview_mode == "Blog Post": st.components.v1.html(build_page("Article", gen_blog_post_html()), height=600, scrolling=True)
    elif preview_mode == "Product Detail": st.components.v1.html(build_page("Product", gen_product_page_content(True)), height=600, scrolling=True)
...
with c2:
    if st.button("DOWNLOAD WEBSITE ZIP", type="primary"):
        ...
        with zipfile.ZipFile(z_b, "a", zipfile.ZIP_DEFLATED, False) as zf:
            ...
            zf.writestr("privacy.html", build_page("Privacy", f"<div class='container' style='padding-top:120px;'>{format_text(priv_txt)}</div>"))
            zf.writestr("terms.html", build_page("Terms", f"<div class='container' style='padding-top:120px;'>{format_text(term_txt)}</div>"))
            ...
            sitemap = f"""<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>{prod_url}/</loc></url><url><loc>{prod_url}/index.html</loc></url><url><loc>{prod_url}/contact.html</loc></url></urlset>"""
